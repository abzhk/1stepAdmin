        ...
        <div className="bg-white rounded-2xl shadow-xl border border-[#8fa797]/20 w-[98vw] max-w-none">
          <div className="flex items-center justify-between px-5 py-4 border-b border-[#8fa797]/15">
            <div>
              ...
              <p className="text-xs text-[#8fa797]">{doc.type} · {doc.size}</p>
            </div>
            <button onClick={onClose} className="p-1.5 hover:bg-[#F6F4F0] rounded-lg transition text-[#8fa797] hover:text-[#2d4a36]">
...
const DetailPanel = ({ applicant, onDecision, onDocStatusChange, onNoteAdd, onFieldStatusChange, onPriorityChange, showToast }) => {
  ...
  const cycleField = (section, field) => {
    ...
    const next = FIELD_STATUS_CYCLE[(FIELD_STATUS_CYCLE.indexOf(cur) + 1) % FIELD_STATUS_CYCLE.length];
  ...
  const liveViewingDoc = viewingDoc ? applicant.docs.find(d => d.id === viewingDoc.id) || null : null;
    ...
    <div className="flex flex-col h-full bg-white">
        ...
        <div className="px-6 pt-5 pb-4 border-b border-[#8fa797]/20 flex-shrink-0">
          <div className="flex items-start gap-3 mb-4">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-sm font-bold flex-shrink-0 ${applicant.color}`}>
              {applicant.initials}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-base font-bold text-[#2d4a36]">{applicant.name}</h2>
                <PriorityBadge priority={applicant.priority}
                  onClick={() => {
                    const order = ["low", "medium", "high"];
                    const next  = order[(order.indexOf(applicant.priority) + 1) % order.length];
                    onPriorityChange(applicant.id, next);
                    showToast(`Priority → ${next}`, "info");
                  }} />
                {isLocked && <span className="text-[10px] text-[#8fa797] italic">Locked</span>}
              </div>
              <p className="text-xs font-medium text-[#2d4a36]/60 mt-0.5">{applicant.role} · {applicant.city}</p>
              <div className="flex items-center gap-3 mt-1 flex-wrap">
                <span className="text-[10px] text-[#8fa797]">{applicant.email}</span>
                <span className="text-[10px] text-[#8fa797]">{applicant.phone}</span>
              </div>
            </div>
            <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
              <StatusBadge status={applicant.overall} />
              <span className="text-[10px] text-[#8fa797]">{timeAgo(applicant.submittedTs)}</span>
            </div>
          </div>

 
          <div className="flex justify-between mb-1">
            <span className="text-[10px] font-medium text-[#8fa797]">Document verification</span>
            <span className="text-[10px] font-bold text-[#2d4a36]/80">{verifiedDocs}/{totalDocs} verified · {progress}%</span>
          </div>
          ...
          <div className="flex gap-2 mb-4">
            ...
            <button onClick={() => { setActiveTab("notes"); setTimeout(() => noteRef.current?.focus(), 80); }}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold border border-[#8fa797]/30 rounded-lg text-[#2d4a36]/80 hover:bg-[#F6F4F0] transition">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <button onClick={() => showToast("Profile link copied", "info")}
